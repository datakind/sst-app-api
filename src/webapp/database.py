...
class InstTable(Base): # type: ignore
    ...
    name = Column(String(VAR_CHAR_STANDARD_LENGTH), nullable=False, unique=True) # type: ignore
    ...
    allowed_emails = Column(MutableDict.as_mutable(JSON)) # type: ignore
    # Schemas that are allowed for validation.
    schemas = Column(MutableList.as_mutable(JSON)) # type: ignore
    state = Column(String(VAR_CHAR_LENGTH), nullable=True) # type: ignore
    # Only populated for PDP schools.
    pdp_id = Column(String(VAR_CHAR_LENGTH), nullable=True) # type: ignore
    ...
    created_by = Column(Uuid(as_uuid=True), nullable=True) # type: ignore
...
class FileTable(Base): # type: ignore
    ...
    __tablename__ = "file"
    name = Column(String(VAR_CHAR_STANDARD_LENGTH), nullable=False) # type: ignore
    id = Column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4) # type: ignore
    ...
    uploader = Column(Uuid(as_uuid=True), nullable=True) # type: ignore
    # Can be PDP_SFTP, MANUAL_UPLOAD etc. May be empty for generated files.
    source = Column(String(VAR_CHAR_LENGTH), nullable=True) # type: ignore
    ...
    deleted_at = Column(DateTime(timezone=True), nullable=True) # type: ignore
    retention_days: Mapped[int] = mapped_column(nullable=True)
...
class BatchTable(Base): # type: ignore
    ...
    name = Column(String(VAR_CHAR_STANDARD_LENGTH), nullable=False) # type: ignore
    created_by = Column(Uuid(as_uuid=True)) # type: ignore
    # If null, the following is non-deleted.
    deleted: Mapped[bool] = mapped_column(nullable=True)
    ...
    updated_by = Column(Uuid(as_uuid=True), nullable=True) # type: ignore
...
class ModelTable(Base): # type: ignore
    ...
    schema_configs = Column(JSON, nullable=True) # type: ignore
    created_by = Column(Uuid(as_uuid=True), nullable=True) # type: ignore
    # If null, the following is non-deleted.
    deleted: Mapped[bool] = mapped_column(nullable=True)
...
class JobTable(Base): # type: ignore
    ...
    # The time the deletion request was set.
    triggered_at = Column(DateTime(timezone=True), nullable=False) # type: ignore
    batch_name = Column(String(VAR_CHAR_STANDARD_LENGTH), nullable=False) # type: ignore
    ...
    output_filename = Column(String(VAR_CHAR_STANDARD_LENGTH), nullable=True) # type: ignore
    # Whether the file was approved.
    output_valid: Mapped[bool] = mapped_column(nullable=True, default=False)
    err_msg = Column(String(VAR_CHAR_STANDARD_LENGTH), nullable=True) # type: ignore
    completed: Mapped[bool] = mapped_column(nullable=True)
