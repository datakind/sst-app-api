...
class InstTable(Base):
    ...
    name : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_STANDARD_LENGTH), nullable=False, unique=True)
    ...
    allowed_emails: Mapped[dict[str, str]] = mapped_column(MutableDict.as_mutable(JSON()))
    # Schemas that are allowed for validation.
    schemas: Mapped[dict[str, str]] = mapped_column(MutableList.as_mutable(JSON()))
    ...
    created_at: Mapped[dict[str, str]] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[dict[str, str]] = mapped_column(DateTime(timezone=True), onupdate=func.now())
...
class ApiKeyTable(Base):
    ...
    hashed_key_value : Mapped[dict[str, str]] = mapped_column(
        String(VAR_CHAR_STANDARD_LENGTH), nullable=False, unique=True
    )

    # Set the foreign key to link to the institution table.
    inst_id : Mapped[dict[str, str]] = mapped_column(
        Uuid(as_uuid=True),
        ForeignKey("inst.id", ondelete="CASCADE"),
        nullable=True,
    )
    inst: Mapped["InstTable"] = relationship(back_populates="apikeys")
    created_by : Mapped[dict[str, str]] = mapped_column(Uuid(as_uuid=True), nullable=False)
    notes : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_STANDARD_LENGTH), nullable=True)
    ...
    access_type : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_LENGTH), nullable=False)
...
class AccountTable(Base):
    ...
    name : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_STANDARD_LENGTH), nullable=False)
    email : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_STANDARD_LENGTH), nullable=False, unique=True)
    google_id : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_STANDARD_LENGTH), nullable=True)
    azure_id : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_STANDARD_LENGTH), nullable=True)

    email_verified_at : Mapped[dict[str, str]] = mapped_column(DateTime(timezone=True), nullable=True)
    password : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_STANDARD_LENGTH), nullable=False)
    two_factor_secret : Mapped[dict[str, str]] = mapped_column(Text, nullable=True)
    two_factor_recovery_codes : Mapped[dict[str, str]] = mapped_column(Text, nullable=True)
    two_factor_confirmed_at : Mapped[dict[str, str]] = mapped_column(DateTime(timezone=True), nullable=True)

    remember_token : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_STANDARD_LENGTH), nullable=True)
...
class AccountHistoryTable(Base):
    ...
    id : Mapped[dict[str, str]] = mapped_column(Integer, primary_key=True)  # Auto-increment should be default
    timestamp : Mapped[dict[str, str]] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )

    # Set the parent foreign key to link to the users table.
    account_id : Mapped[dict[str, str]] = mapped_column(
        Uuid(as_uuid=True),
    ...
    action : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_STANDARD_LENGTH), nullable=False)
    resource_id : Mapped[dict[str, str]] = mapped_column(Uuid(as_uuid=True), nullable=False)
...
class FileTable(Base):
    ...
    schemas : Mapped[dict[str, str]] = mapped_column(MutableList.as_mutable((JSON())), nullable=False)  
    ...
    deleted_at : Mapped[dict[str, str]] = mapped_column(DateTime(timezone=True), nullable=True)  
    retention_days: Mapped[int] = mapped_column(nullable=True)
...
class BatchTable(Base):
    ...
    name : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_STANDARD_LENGTH), nullable=False)  
    created_by : Mapped[dict[str, str]] = mapped_column(Uuid(as_uuid=True))  
    # If null, the following is non-deleted.
    deleted: Mapped[bool] = mapped_column(nullable=True)
...
class ModelTable(Base):
    ...
    version : Mapped[dict[str, str]] = mapped_column(Integer, default=0)  
...
class JobTable(Base):
    ...
    __tablename__ = "job"
    id : Mapped[dict[str, str]] = mapped_column(BigInteger, primary_key=True)

    # Set the parent foreign key to link to the institution table.
    model_id : Mapped[dict[str, str]] = mapped_column(
        Uuid(as_uuid=True),
    ...
    created_by : Mapped[dict[str, str]] = mapped_column(Uuid(as_uuid=True), nullable=False)  
    # The time the deletion request was set.
    triggered_at : Mapped[dict[str, str]] = mapped_column(DateTime(timezone=True), nullable=False)  
    batch_name : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_STANDARD_LENGTH), nullable=False)  
    ...
    output_filename : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_STANDARD_LENGTH), nullable=True)  
    # Whether the file was approved.
    output_valid: Mapped[bool] = mapped_column(nullable=True, default=False)
    err_msg : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_STANDARD_LENGTH), nullable=True)  
    completed: Mapped[bool] = mapped_column(nullable=True)
...
class SchemaRegistryTable(Base):
    ...
    version_label : Mapped[dict[str, str]] = mapped_column(String(VAR_CHAR_STANDARD_LENGTH), nullable=False)
    ...
    created_at : Mapped[dict[str, str]] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )  
